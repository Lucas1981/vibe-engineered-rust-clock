use crate::layout::{NUMERAL_FONT_PX, NUMERAL_RADIUS_FRAC};
use fontdue::{Font, Metrics};
use std::f32::consts::{FRAC_PI_2, TAU};
use tiny_skia::Pixmap;

/// Place numerals 1–12 around the clock face at `NUMERAL_RADIUS_FRAC` of `radius`.
pub fn draw_numerals(pixmap: &mut Pixmap, font: &Font, cx: f32, cy: f32, radius: f32) {
    let num_radius = radius * NUMERAL_RADIUS_FRAC;
...
fn draw_str(pixmap: &mut Pixmap, font: &Font, text: &str, cx: f32, cy: f32) {
    ...
    let glyphs: Vec<(Metrics, Vec<u8>)> = text
        .chars()
        .map(|c| font.rasterize(c, NUMERAL_FONT_PX))
        .collect();
